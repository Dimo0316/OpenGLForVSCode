...
#pragma region Usingframe buffer
    unsigned int framebuffer; 
    glGenFramebuffers(1, &framebuffer); // 创建 framebuffer
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer); // 绑定 framebuffer
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, 0); // 解绑操作 激活默认帧缓冲
#pragma endregion
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < grassPositions.size(); ++i){
            ...
            stored[distance] = grassPositions[i];
        }

        for (std::map<float, glm::vec3>::reverse_iterator iterator = stored.rbegin(); iterator != stored.rend(); iterator++)
        {
            model = glm::mat4(1.0f);
...
#pragma region Using frame buffer
